...
- **`scrape_jobs.yml`**: At 00:05 and hourly from 09:05 through 21:05 America/New_York, serially orchestrates the single LinkedIn worker, a bounded Freehire compatibility increment (hard limit `300`), and source publication finalization. Search and detail requests share one configurable global cadence; `max_pages_per_query` is an exact page count. Manual recovery may set one `SCRAPE_ARCHETYPE`, while scheduled runs leave it blank and scrape every enabled lane. Publication pruning is bounded maintenance outside the atomic generation switch.
