...
def test_run_is_dry_run_by_default_behavior(monkeypatch):
    ...
    monkeypatch.setattr(backfill.scraper, "_fetch_linkedin_job_details", lambda _job_id, **_kwargs: ({"location": "Toronto"}, {}))
...
def test_run_applies_multiple_instances_from_same_canonical_row(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda job_id, **_kwargs: ({"location": "Toronto" if job_id == "source-1" else "Calgary"}, {}),
    )
